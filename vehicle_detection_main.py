...
def object_detection_function(command):
    ...
    if command == "imwrite":
        ...
        output_movie = cv2.VideoWriter(
            source_video.split(".")[0] + "_output.avi", fourcc, fps, (width, height)
        )
...
parser = argparse.ArgumentParser(description="Transport Detection TensorFlow.")
parser.add_argument("command", metavar="<command>", help="'imshow' or 'imwrite'")
