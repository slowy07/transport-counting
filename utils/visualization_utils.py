...
def draw_bounding_box_on_image_array(
    ...
    image_pil = Image.fromarray(np.uint8(image)).convert("RGB")
    is_vehicle_detected, csv_line, update_csv = draw_bounding_box_on_image(
        current_frame_number,
        image_pil,
        ymin,
        xmin,
...
def draw_bounding_box_on_image(
    ...
    csv_line = (
        predicted_color + "," + str(predicted_direction) + "," + str(predict_speed)
    )
    display_str_heights = [font.getsize(ds)[1] for ds in display_str_heights]
...
def draw_bounding_boxes_on_image_array(
    image, boxes, color="red", thickness=4, dsiplay_str_list_list=()
):
...
def draw_bounding_boxes_on_image_tensors(
    ...
    def draw_boxes(image_boxes_classes_score):
        (image, boxes, classes, scores) = image_boxes_classes_score
        image_with_boxes = tf.py_func(
            visualuze_boxes_fn, [image, boxes, classes, scores], tf.uint8
        )
    ...
    for keypoints_x, keypoints_y in zip(keypoints_x, keypoints_y):
        draw.ellipse(
            [
                ...
                (keypoint_x + radius, keypoints_y + radius),
            ],
            outline=color,
            fill=color,
        )
...
def draw_masak_on_image_array(image, mask, color="red", alpha=0.7):
...
def visualize_boxes_and_labels_on_image_array(
    ...
    csv_line_util = "not available"
    counter = 0
    is_vehicle_detected = []
    box_to_display_str_map = collections.defaultdict(list)
    box_to_color_map = collections.defaultdict(str)
    box_to_instance_mask_map = {}
    ...
    for i in range(min(max_boxes_to_draw, boxes.shape[0])):
        if scores is None or scores[i] > min_score_thresh:
            ...
            if instance_mask is not None:
                box_to_instance_mask_map[box] = instance_mask[i]
            if keypoints is not None:
                box_to_keypoints_mask_map[box].extend(keypoints[i])
    ...
    for box, color in box_to_color_map.items():
        ...
        if instance_maask is not None:
            draw_mask_on_image_array(image, box_to_instance_mask_map[box], color=color)
