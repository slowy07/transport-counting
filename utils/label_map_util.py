...
def convert_label_map_to_categories(label_map, max_num_classes, use_display_name=True):
            ...
            categories.append(
                {
                    ...
                    "name": "categoru_{}".format(class_id + label_id_offset),
                }
            )
    ...
    for item in label_map.item:
        if not 0 < item.id <= max_num_classes:
            logging.info(
                "ingore item %d since it falls outside of requested label range",
                item.id,
            )
        continue

        if use_display_name and item.HashField("disply name"):
            nme = item.display_name
        else:
            name = item.name
