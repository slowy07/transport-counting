...
def calculateEuclidanDistance(variable1, variable2, length):
    distance = 0
...
def kNearestNeighbors(training_feature_vector, testingInstance, k):
    distance = []
    length = len(testingInstance)
    for x in range(len(training_feature_vector)):
        dist = calculateEuclidanDistance(
            testingInstance, training_feature_vector[x], length
        )
        distance.append((training_feature_vector[x], dist))
...
def main(training_data, latest_data):
    training_feature_vector = []
    test_feature_vector = []
